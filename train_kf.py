...
def main():
    ...
    for fold, (train_ids, test_ids) in enumerate(kfold.split(main_dataset)):
        ...
        dataloaders = {x: DataLoader(main_dataset, batch_size=int(conf.TRAIN.batch_size), sampler = data_pack[x], pin_memory=True) for x in ['train', 'val']} 
        ...
        for epoch in range(num_epochs):
            ...
            logging.info("Fold {} - Epoch {}/{} - LR: {:.5f} - Train Loss: {:.10f} and Validation Loss: {:.10f} and Training Accuracy: {:.5f} and Validation Accuracy: {:.5f}".format(fold+1, epoch+1, num_epochs, scheduler.get_last_lr()[0], phase_error['train'], phase_error['val'], phase_accuracy['train'], phase_accuracy['val']))
